...
The TruffulaPrinterTest class now includes many tests covering hidden-file behavior, color behavior, and Wave 7 alphabetization (nested files, directories, and mixed content). It uses helper methods for hidden files on Unix and Windows, and the tests verify correct ordering and indentation.

## AlphabeticalFileSorter.java
The AlphabeticalFileSorter class is a utility class with a method sortFiles(File[] files) that takes an array of File objects and sorts them in a case-insensitive alphabetical order. The method uses Arrays.sort() with compareToIgnoreCase() on file names. On Windows, the file system is case-insensitive, so names like Cat.png and cat.png are treated as the same file and cannot be reliably tested as separate files in the same folder. This class is used by TruffulaPrinter to ensure the ouput is alphabetized.